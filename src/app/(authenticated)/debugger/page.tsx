...
interface ParamDef {
  ...
  // Condition: only show when another param has a specific value
...
const ACTIONS: ActionDef[] = [
  ...
  { name: 'send_msg', label: '发送消息', description: '发送消息（私聊/群聊）', params: [
    ...
    { key: 'user_id', label: '用户 QQ 号', type: 'text', required: true, default: '', placeholder: '目标 QQ 号', showWhen: { param: 'message_type', value: 'private' } },
    { key: 'group_id', label: '群号', type: 'text', required: true, default: '', placeholder: '目标群号', showWhen: { param: 'message_type', value: 'group' } },
    { key: 'message', label: '消息内容', type: 'textarea', required: true, default: '', placeholder: '输入消息文本...' },
    { key: 'auto_escape', label: '纯文本发送', type: 'boolean', required: false, default: false, description: '不解析 CQ 码' },
  ]},
  { name: 'delete_msg', label: '撤回消息', description: '撤回一条消息', params: [
    { key: 'message_id', label: '消息 ID', type: 'number', required: true, default: 0, placeholder: '消息 ID' },
  ]},
  { name: 'get_group_msg_history', label: '获取群历史消息', description: '获取指定群聊的历史消息', params: [
    ...
    { key: 'message_seq', label: '起始序号', type: 'number', required: false, default: 0, placeholder: '从最新开始' },
  ]},
  ...
  { name: 'delete_friend', label: '删除好友', description: '删除指定好友', params: [
    { key: 'user_id', label: '用户 QQ 号', type: 'text', required: true, default: '', placeholder: '输入 QQ 号' },
    { key: 'block', label: '同时拉黑', type: 'boolean', required: false, default: false },
  ]},
  { name: 'set_friend_add_request', label: '处理好友请求', description: '同意或拒绝好友请求', params: [
    { key: 'flag', label: '请求 flag', type: 'text', required: true, default: '', placeholder: '从事件中获取' },
    { key: 'approve', label: '同意请求', type: 'boolean', required: false, default: true },
  ...
  { name: 'set_group_card', label: '设置群名片', description: '设置群成员名片', params: [
    { key: 'group_id', label: '群号', type: 'text', required: true, default: '', placeholder: '输入群号' },
    { key: 'user_id', label: '用户 QQ 号', type: 'text', required: true, default: '', placeholder: '输入 QQ 号' },
    { key: 'card', label: '群名片', type: 'text', required: false, default: '', placeholder: '留空则清除' },
  ]},
  ...
  { name: 'set_group_ban', label: '群禁言', description: '禁言指定群成员', params: [
    { key: 'group_id', label: '群号', type: 'text', required: true, default: '', placeholder: '输入群号' },
    { key: 'user_id', label: '用户 QQ 号', type: 'text', required: true, default: '', placeholder: '输入 QQ 号' },
    { key: 'duration', label: '禁言时长(秒)', type: 'number', required: false, default: 60, placeholder: '0=解除禁言' },
  ]},
  { name: 'set_group_whole_ban', label: '全员禁言', description: '开启或关闭全员禁言', params: [
    { key: 'group_id', label: '群号', type: 'text', required: true, default: '', placeholder: '输入群号' },
    { key: 'enable', label: '开启全员禁言', type: 'boolean', required: false, default: true },
  ]},
  { name: 'set_group_leave', label: '退出群聊', description: '退出指定群聊', params: [
    { key: 'group_id', label: '群号', type: 'text', required: true, default: '', placeholder: '输入群号' },
    { key: 'is_dismiss', label: '解散群', type: 'boolean', required: false, default: false },
  ]},
  ...
  { name: 'set_group_admin', label: '设置管理员', description: '设置或取消群管理员', params: [
    { key: 'group_id', label: '群号', type: 'text', required: true, default: '', placeholder: '输入群号' },
    { key: 'user_id', label: '用户 QQ 号', type: 'text', required: true, default: '', placeholder: '输入 QQ 号' },
    { key: 'enable', label: '设为管理', type: 'boolean', required: false, default: true },
  ]},
  ...
  { name: 'set_online_status', label: '设置在线状态', description: '设置机器人在线状态', params: [
    ...
    { key: 'battery_status', label: '电量百分比', type: 'number', required: false, default: 100, placeholder: '100' },
  ]},
  ...
  { name: 'get_record', label: '获取语音', description: '获取语音文件信息', params: [
    ...
    { key: 'out_format', label: '输出格式', type: 'select', required: false, default: 'mp3', options: [
      { value: 'mp3', label: 'mp3' }, { value: 'amr', label: 'amr' }, { value: 'wav', label: 'wav' }, { value: 'flac', label: 'flac' },
    ]},
  ]},
  ...
  { name: 'download_file', label: '下载文件', description: '下载文件到本地', params: [
    ...
    { key: 'headers', label: '请求头', type: 'textarea', required: false, default: '', placeholder: 'JSON 格式，可选' },
  ]},
  ...
  { name: 'get_group_honor_info', label: '获取群荣誉', description: '获取群荣誉信息', params: [
    { key: 'group_id', label: '群号', type: 'text', required: true, default: '', placeholder: '输入群号' },
  ...
  { name: 'get_group_system_msg', label: '获取群系统消息', description: '获取群系统消息', params: [] },
  { name: 'get_online_clients', label: '获取在线客户端', description: '获取在线客户端列表', params: [
    { key: 'no_cache', label: '不使用缓存', type: 'boolean', required: false, default: false },
  ]},
...
export default function DebuggerPage() {
  ...
  useState(() => { handleActionChange(ACTIONS[0].name) })

  // Check if a param should be visible based on showWhen condition
  const isParamVisible = (p: ParamDef): boolean => {
    if (!p.showWhen) return true
    return formValues[p.showWhen.param] === p.showWhen.value
  }
  ...
  const buildParams = (): Record<string, unknown> => {
    ...
    for (const p of selectedAction.params) {
      ...
      if (isRequired || includedOptional[p.key]) {
  ...
  const replayEntry = (entry: DebugEntry) => {
    ...
    if (action) {
      ...
      const included: Record<string, boolean> = {}
      for (const p of action.params) {
        if (!p.required) included[p.key] = entry.params[p.key] !== undefined
      }
      setIncludedOptional(included)
    }
            ...
            <div className="p-4 space-y-4">
                  ...
                  {ACTION_GROUPS.map((group) => (
                    <optgroup key={group.label} label={group.label}>
                      {group.actions.map((name) => {
                        ...
                        return action ? <option key={name} value={name}>{action.label} ({name})</option> : null
                      })}
                    </optgroup>
                  ))}
                </select>
              ...
              {selectedAction.params.length > 0 && (
                <div className="rounded-lg border overflow-hidden">
                  {/* Header */}
                  <div className="grid grid-cols-[1fr_2fr_40px] border-b bg-muted/50 text-xs font-medium text-muted-foreground">
                    <div className="px-3 py-2">参数名</div>
                    <div className="px-3 py-2">值</div>
                    <div className="px-3 py-2 text-center">发送</div>
                  </div>

                  {/* Rows */}
                  {selectedAction.params.map((p) => {
                    const isVisible = isParamVisible(p)
                    ...
                    return (
                      <div
                        key={p.key}
                        className={`grid grid-cols-[1fr_2fr_40px] border-b last:border-b-0 items-center ${!isIncluded ? 'opacity-40' : ''}`}
                      >
                        {/* Param name */}
                        ...
                        <div className="px-3 py-2">
                          {isIncluded ? (
                            p.type === 'select' ? (
                              ...
                              <label className="flex items-center gap-2 text-sm cursor-pointer">
                                <input
                                  ...
                                  className="h-4 w-4"
                                />
                                <span className="text-muted-foreground">{p.description || '启用'}</span>
                              </label>
                            ) : p.type === 'textarea' ? (
                              <textarea
                                ...
                                rows={3}
                                className="flex w-full rounded-md border border-input bg-background px-2 py-1.5 text-sm resize-none"
                              />
                            ) : p.type === 'number' ? (
                              <input
                                type="number"
                            ...
                            <span className="text-xs text-muted-foreground italic">未启用</span>
                          )}
                        </div>
                        ...
                        <div className="flex justify-center py-2.5">
                          <input
                            ...
                            title={isRequired ? '必填参数' : isIncluded ? '点击取消发送' : '点击添加到请求'}
                            className={`h-4 w-4 ${isRequired ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}
                          />
                        </div>
                      </div>
                    )
              ...
              <div className="flex items-center gap-2">
                <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
                  <input type="checkbox" checked={showRawJson} onChange={(e) => setShowRawJson(e.target.checked)} className="h-3 w-3" />
                  显示原始 JSON
                </label>
              </div>
              {showRawJson && (
                <pre className="rounded-lg border p-3 text-xs font-mono overflow-auto max-h-40 bg-muted/30">
              ...
              <button
                onClick={handleSend}
                disabled={sending}
                className="inline-flex w-full items-center justify-center rounded-md bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
              >
                {sending ? '发送中...' : '发送请求'}
              </button>
            </div>
              ...
              {response ? (
                <div className="space-y-3">
                  {(response as Record<string, unknown>)?.status === 'ok' ? (
                    <div className="rounded-md bg-green-50 border border-green-200 px-3 py-2 text-sm text-green-800">✅ 请求成功</div>
                  ) : (
                    <div className="rounded-md bg-red-50 border border-red-200 px-3 py-2 text-sm text-red-800">
                      ❌ {(response as Record<string, unknown>)?.message as string || '请求失败'}
                    </div>
                  )}
                  <pre className="max-h-[500px] overflow-auto rounded-lg bg-muted p-4 text-xs font-mono">
                    {JSON.stringify(response, null, 2)}
                  </pre>
                </div>
              ) : (
                <p className="py-8 text-center text-muted-foreground">发送请求后查看响应</p>
              )}
            </div>
          </div>
                ...
                {history.map((entry) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs ${(entry.response as Record<string, unknown>)?.status === 'ok' ? 'text-green-600' : 'text-destructive'}`}>
